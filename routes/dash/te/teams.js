...
/**
 * GET /dash/te/teams/[_id_]
 */
router.get("/:id?", async (req, res) => {
    const buildid = req.params.id ? parseInt(req.params.id) : undefined;
    const periods = period.select("dashte");
    const build = await bl.main.get(); //TODO: Obtener info de bd para edificios
    const data = { build };
    const buildname = build.find(o => o.ID === buildid)?.Name ?? "Microsoft Teams - Infraestructura";
